...
class RegisterActivity : AppCompatActivity() {
            ...
            if (email.isNotEmpty() && pass.isNotEmpty() && confirmPass.isNotEmpty() && phoneNumber.isNotEmpty()) {
                if (pass == confirmPass) {
                    if (isValidPassword(pass)) {
                        ...
                        Toast.makeText(this, "Password must contain at least 1 uppercase, 1 lowercase, 1 special character (@#$&), and 1 numeric digit.", Toast.LENGTH_LONG).show()
                    }
                } else {
                    Toast.makeText(this, "Password is not matching", Toast.LENGTH_SHORT).show()
                }
        ...
        firebaseAuth.signInWithCredential(credential)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    ...
                    saveUserData(user?.uid, user?.email, user?.phoneNumber)
                } else {
   ...
   /* private fun savePhoneNumber(userId: String?, phoneNumber: String?) {
    ...
    private fun saveUserData(userId: String?, email: String?, phoneNumber: String?) {
        userId?.let {
            val database = FirebaseDatabase.getInstance()
            val usersRef = database.getReference("Users")
